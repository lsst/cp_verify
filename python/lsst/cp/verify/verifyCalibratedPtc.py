...
__all__ = ['CpVerifyCalibratedPtcConfig', 'CpVerifyCalibratedPtcTask']


class CpVerifyCalibratedPtcConfig(CpVerifyCalibConfig,
                                  pipelineConnections=CpVerifyCalibConnections):
    """Inherits from base CpVerifyCalibConfig."""

    gainThreshold = pexConfig.Field(
        dtype=float,
        doc="Maximum percentage difference between calibrated PTC gain and 1.",
        default=1.0,
    )
    ...
    def setDefaults(self):
        super().setDefaults()
        self.stageName = 'CALIBRATED_PTC'


class CpVerifyCalibratedPtcTask(CpVerifyCalibTask):
    """PTC verification sub-class, implementing the verify method.
    """
    ConfigClass = CpVerifyCalibratedPtcConfig
    _DefaultName = 'cpVerifyCalibratedPtc'
    ...
    def amplifierStatistics(self, inputCalib, camera=None):
        ...
        for amp in detector:
            ...
            outputStatistics[ampName]['PTC_NOISE'] = amp.getReadNoise()
            ...
            if ptcFitType == 'FULLCOVARIANCE':
                ...
                outputStatistics[ampName]['PTC_BFE_A10'] = float(inputCalib.aMatrix[ampName][1][0])
                outputStatistics[ampName]['PTC_BFE_A01'] = float(inputCalib.aMatrix[ampName][0][1])
